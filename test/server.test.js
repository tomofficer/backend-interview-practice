...
describe("Todo API:", function () {
  ...
  describe("GET /v1/todos/:todo_id", () => {
    ...
    it("should return correct todo when given an id", () => {
      ...
      return db("todo")
        ...
        .then((_doc) => {
          doc = _doc;
          return supertest(app).get(`/v1/todos/${doc.id}`).expect(200);
        })
        .then((res) => {
    ...
    it("should respond with a 404 when given an invalid id", () => {
      return supertest(app).get("/v1/todos/aaaaaaaaaaaa").expect(404);
    });
  });
  ...
  describe("POST /v1/todos", function () {
    ...
    it("should respond with 400 status when given bad data", function () {
      ...
      return supertest(app).post("/v1/todos").send(badItem).expect(400);
    });
  });

  describe("PATCH /v1/todos/:todo_id", () => {
    beforeEach("insert some todos", () => {
      return db("todo").insert(todos);
    });
  ...
  describe("DELETE /v1/todos/:todo_id", () => {
    beforeEach("insert some todos", () => {
      return db("todo").insert(todos);
    });
    ...
    it("should delete an item by id", () => {
      return db("todo")
        .first()
        .then((doc) => {
          return supertest(app).delete(`/v1/todos/${doc.id}`).expect(204);
        });
    });
